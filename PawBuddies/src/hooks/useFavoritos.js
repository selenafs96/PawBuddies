...
export const useFavoritos = () => {
  ...
  async function toggleFavorito(id_animal) {
    ...
    if (esFavorito) {
      ...
      setAnimalesFavoritos(prev => prev.filter(a => a.id_animal !== id_animal));
    } else {
      // Añadir a favoritos
      await supabase
        .from('favorito')
        .insert({ id_usuario: ID_USUARIO_PRUEBA, id_animal });
  ...
  return { favoritos, animalesFavoritos, loading, toggleFavorito, eliminarFavorito, fetchFavoritos };
};